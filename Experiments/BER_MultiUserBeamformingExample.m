...
if ~exist(fullfile(tempdir,'BER_TxBits.mat'),'file')
    ...
    M = 64;  k = log2(M);
    data_64QAM = randi([0 1],64*k,1);
    % Create sequence of bits for 32QAM
    M = 16;  k = log2(M);
    data_32QAM = randi([0 1],64*k,1);
    % Create sequence of bits for 16QAM
    M = 16;  k = log2(M);
    data_16QAM = randi([0 1],64*k,1);
    % Create sequence of bits for 8QAM
    M = 16;  k = log2(M);
    data_8QAM = randi([0 1],64*k,1);
    % Create sequence of bits for QPSK
    M = 16;  k = log2(M);
    data_QPSK = randi([0 1],64*k,1);
    % Create sequence of bits for BPSK
    M = 16;  k = log2(M);
    data_BPSK = randi([0 1],64*k,1);
    % Create sequence of bits for QPSK  - Extras
    M = 4;  k = log2(M);
    data_QPSK_extra = randi([0 1],64*k,2);
%     % Create sequence of bits for QPSK
%     M = 4;  k = log2(M);
%     data_QPSK = randi([0 1],64*k,7);
    % Save
    save(fullfile(tempdir,'BER_TxBits.mat'),'data_64QAM','data_16QAM','data_QPSK');
else
    load(fullfile(tempdir,'BER_TxBits.mat'),'data_64QAM','data_16QAM','data_QPSK');
end

% Create 64-QAM symbols
sym_64QAM = qammod(data_64QAM,64,'InputType','bit','UnitAveragePower',true);
sym_64QAM = sym_64QAM.';
% Create 32-QAM symbols
sym_32QAM = qammod(data_32QAM,32,'InputType','bit','UnitAveragePower',true);
sym_32QAM = sym_32QAM.';
% Create 16-QAM symbols
sym_16QAM = qammod(data_16QAM,16,'InputType','bit','UnitAveragePower',true);
sym_16QAM = sym_16QAM.';
% Create 8-QAM symbols
sym_8QAM = qammod(data_8QAM,8,'InputType','bit','UnitAveragePower',true);
sym_8QAM = sym_8QAM.';
% Create QPSK symbols
sym_QPSK = qammod(data_QPSK,4,'InputType','bit','UnitAveragePower',true);
sym_QPSK = sym_QPSK.';
% Create QPSK symbols
sym_BPSK = qammod(data_QPSK,4,'InputType','bit','UnitAveragePower',true);
sym_QPSK = sym_QPSK.';
% Create QPSK symbols
sym_QPSK = qammod(data_QPSK_extra,4,'InputType','bit','UnitAveragePower',true);
sym_QPSK = sym_QPSK.';
...
constDiagram(sym_64QAM.');        % Plot constellation
